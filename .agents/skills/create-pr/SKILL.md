...
- 必要なら GitHub 認証を git に設定する(HTTPS で push できない場合)。
  ```bash
  gh auth setup-git
  ```
- 現在のブランチを push する。
...
- 署名やフッター(`🤖 Generated with ...` など)は **付けない**。

### 5. PR 作成

- `gh` で PR を作成する。Draft の場合のみ `--draft` を付ける。
  ```bash
  gh pr create [--draft] --base <base> --title "<日本語タイトル>" --body "<日本語本文>"
  ```
