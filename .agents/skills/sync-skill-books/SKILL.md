...
**実ファイルとしてコピー(ベンダリング)** して同期する。毎回リポジトリを fresh clone する
環境でも、スキルが実体として存在するので確実に discover される。
...
- ユーザーの依頼に `--manifest <path>` があればマニフェストの場所を上書きする(既定 `scripts/skill-books.manifest.json`)。
...
- 導入先リポジトリのルートで実行する。
- 作業ツリーがクリーンであること。未コミット変更があれば `--check` を含めて停止する。
- マニフェスト(`scripts/skill-books.manifest.json`)が存在し、`repository` / `ref` / `sync[]` を持つこと。
- `git` が利用可能で、対象の skill-books リポジトリを clone できること。
- リポジトリの `AGENTS.md` と、対象ファイルに適用される下位の `AGENTS.md` を確認する。
- PR 作成には接続済みGitHubアプリ、導入先の `create-pr` スキル、または認証済みの `gh` を使う。
...
```bash
node .agents/skills/sync-skill-books/sync.mjs --check
```
...
git switch -c codex/sync-skills/$(date +%Y%m%d-%H%M%S) origin/<base>
```
...
- **削除候補のスキル**(同期元に指定スキルが無い場合、ローカルを削除せずエラー終了する)
...
接続済みGitHubアプリが使える場合はそれを優先し、導入先の `create-pr` スキル、
`gh` の順に利用可能な手段で同じ base / タイトル / 本文の PR を作成する。
いずれの手段も無い場合は push 済みブランチを案内し、PR 作成は人間に委ねる。
...
- **`<base>`(デフォルトブランチ)への直接 push は禁止**。push してよいのは `codex/sync-skills/*` のみ。
